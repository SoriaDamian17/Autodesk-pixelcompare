(function () {
   ...
   */
  proto.compareTwoViewables = function (
    viewablePathA,
    optionsA,
    viewablePathB,
    optionsB,
   ...
   * @returns {Promise} that resolves when all models are loaded
   */
  proto.compareTwoDocuments = function (
    docA,
    itemIdA,
    docB,
    itemIdB,
  ...
  proto._compareTwoThings = function (
    thingLoader,
    thingA,
    parameterA,
    thingB,
    ...
    return this[thingLoader](thingA, parameterA, true, function (modelA) {
      if (!modelA || !modelA.getData().isLeaflet) {
        avp.logger.error(
          "No model loaded to compare to or model is not leaflet"
        );
        return false;
      }
      self.modelA = modelA;
      ...
      .then(function () {
        ...
        return self[thingLoader](
          thingB,
          parameterB,
          false,
          self.onSecondModelLoadedBinded
        );
      })
      .then(function () {
  ...
  proto._addSplitLine = function () {
    ...
    if (this.splitLineStyle) {
      ...
      if (width) {
        ...
        this.splitLineContainer.style.left =
          "calc(50% - " + containerWidth / 2 + "px)";
    ...
    var handleMove = function (e) {
      ...
      var adjustedPos = utils.getClientCoords(e).x - rect.left;
      ...
      if (limit < posPercentage && posPercentage < 100 - limit) {
        this.splitLineContainer.style.left =
          "calc(" + posPercentage + "% - " + splitWidth / 2 + "px)";
  ...
  proto._getViewable = function (document, guid) {
    ...
    if (loader !== avp.LeafletLoader) {
      avp.logger.error(
        ...
        av.errorCodeString(av.ErrorCodes.UNSUPORTED_FILE_EXTENSION)
      );
      return null;
    }
  ...
  proto._loadViewable = function (viewablePath, options, addModel, cb) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var loadWithoutAddingModel = function () {
  ...
  proto._loadModel = function (urn, itemId, addModel, cb) {
    ...
    return new Promise(function (resolve, reject) {
      var onLoad = function (doc) {
        self
          ._loadDoc(doc, itemId, addModel, cb)
          .then(function () {
            resolve(true);
          })
          .catch(function (err) {
...
(function () {
  ...
  function _getInputEvents(type) {
    if (av.isIE11) return [inputMap[type]["pointer"]];

    var events = [];
    if (!av.isMobileDevice()) events.push(inputMap[type]["mouse"]);

    if (av.isTouchDevice()) events.push(inputMap[type]["touch"]);

    return events;
  }

  function getClientCoords(event) {
    if (av.isIE11) return { x: event.clientX, y: event.clientY };
  ...
  function isTouchEvent(event) {
    if (av.isIE11) return event.pointerType === "touch";
